...
class TitleRepository(val network: MainNetwork, val titleDao: TitleDao) {
     ...
     * the current tile.
     */
    fun refreshTitleWithCallbacks(titleRefreshCallbackSwitchBoard: TitleRefreshCallbackSwitchBoard) {
        // This request will be run on a background thread by retrofit
        BACKGROUND.submit {
            try {
                // Make network request using a blocking call
                val result = network.fetchNextTitle().execute()//implementation
                if (result.isSuccessful) {
                    // Save it to database
                    titleDao.insertTitle(Title(result.body()!!))
                    // Inform the caller the refresh is completed
                    titleRefreshCallbackSwitchBoard.onCompletedSwitchOne() //switch on
                } else {
                    // If it's not successful, inform the callback of the error
                    titleRefreshCallbackSwitchBoard.onErrorSwitchTwo(//playing with the switches
                        TitleRefreshError("Unable to refresh title", null)
                    )
                }
            } catch (cause: Throwable) {
                // If anything throws an exception, inform the caller
                titleRefreshCallbackSwitchBoard.onErrorSwitchTwo(
...
interface TitleRefreshCallbackSwitchBoard {
    fun onCompletedSwitchOne()
    fun onErrorSwitchTwo(cause: Throwable)
}
